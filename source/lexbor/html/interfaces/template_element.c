...
{
    lxb_dom_document_fragment_t *content = template_element->content;
    lxb_dom_node_t *result = lxb_dom_node_interface_destroy(lxb_dom_interface_node(template_element));
    if (result == NULL) {
        (void) lxb_dom_document_fragment_interface_destroy(content);
    }

    return (lxb_html_template_element_t*) result;
}
